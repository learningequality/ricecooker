...
class SushiChef(object):
    ...
    def parse_args_and_options(self):
        ...
        self._warn_deprecated_flags(args)
    ...
    DEPRECATED_FLAG_WARNINGS = {
        "stage_deprecated": "--stage is now the default bevavior. The --stage flag",
        "reset_deprecated": "--reset is now the default bevavior. The --reset flag",
        "thumbnails_deprecated": (
            "thumbnails are now always generated for content nodes and topics that do not provide one. The --thumbnails flag"
        ),
    }

    def _warn_deprecated_flags(self, args):
        for arg, message in self.DEPRECATED_FLAG_WARNINGS.items():
            if args[arg]:
                config.LOGGER.warning(
                    "DEPRECATION WARNING: {} has been deprecated and will be removed in ricecooker 1.0.".format(
                        message
                    )
                )
