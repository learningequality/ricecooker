        ...
        "files": [
            {
                ...
                "ffmpeg_settings": {"max_width": 480, "clf": 20},
            }
        ],
    },
    {
    ...
    },
    {
        ...
        "children": [
            ...
            # },
            {
                ...
                "files": [
                    {
                        "path": "C:/users/jordan/Videos/testfolder/HTML Test.zip",
                    }
                ]
            },
            # {
...
def add_files(node, file_list):
    for f in file_list:
        file_type = files.guess_file_type(f['path'], node.kind)

        if file_type == files.FileTypes.AUDIO_FILE:
            node.add_file(files.AudioFile(path=f['path']))
            ...
            node.add_file(files.DocumentFile(path=f['path']))
        elif file_type == files.FileTypes.HTML_ZIP_FILE:
            node.add_file(files.HTMLZipFile(path=f['path']))
        else:
