...
from ricecooker.classes.files import FileTypes, guess_file_type, AudioFile, ThumbnailFile, DocumentFile
...
def construct_channel(**kwargs):
    ...
    channel_thumbnail = ThumbnailFile(
...
def _build_tree(node, sourcetree):
    ...
    for child_source_node in sourcetree:
        try:
            ...
            kind = guess_content_kind(files, child_source_node.get("questions"))
...
def add_files(node, files):
    for f in files:
        ...
        if file_type == FileTypes.AUDIO_FILE:
            node.add_file(AudioFile(path=f['path']))
        elif file_type == FileTypes.THUMBNAIL:
            node.add_file(ThumbnailFile(path=f['path']))
        elif file_type == FileTypes.DOCUMENT_FILE:
            node.add_file(DocumentFile(path=f['path']))
        else:
