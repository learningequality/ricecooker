...
from ricecooker.utils.images import create_image_from_html_zip
...
class ThumbnailContextMetadata(ContextMetadata):
    ...
    content_node_metadata: Optional[Dict] = None
...
class ThumbnailExtractionHandler(ExtensionMatchingHandler):
    ...
    def get_file_kwargs(self, context):
        decomposed = (context.content_node_metadata or {}).get("children") is not None
        if context.node_has_thumbnail or decomposed:
    ...
    def handle_file(self, path):
        ...
        with self.write_file(file_formats.PNG) as fh:
            if ext == file_formats.PDF:
                ...
                create_image_from_html_zip(path, fh.name)
            else:
