...
class _HTMLReferenceRewriter(HTMLParser):
    ...
    ``style`` and ``<style>`` block text — recording each as a
    ``(start, end, replacement)`` span in the original string rather than mutating
    a parsed tree, so unmatched bytes survive verbatim. Navigation references
    (``<a href>``, ``<iframe src>``) are excluded: they are page links, not
    offline resources.
    """
