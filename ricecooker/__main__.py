...
"""Usage: ricecooker uploadchannel [-huv] <file_path> [--warn] [--compress] [--token=<t>] [--max-retries=<n>] [--resume [--step=<step>] | --reset] [--prompt] [--publish] [[OPTIONS] ...]
...
if __name__ == '__main__':
    ...
    try:
      int(arguments['--max-retries'])
    except ValueError:
      raise InvalidUsageException("Invalid argument: Max-retries must be an integer.")


    uploadchannel(arguments["<file_path>"],
                  verbose=arguments["-v"],
                  update=arguments['-u'],
                  max_retries=arguments['--max-retries'],
