...
def test_validate(
    ...
    contentnode_invalid_files,
    contentnode_no_source_id,
    video,
    video_invalid_files,
    audio,
    audio_invalid_files,
    document,
    document_invalid_files,
    html,
    html_invalid_files,
    html_invalid_zip,
    exercise,
...
def test_alternative_domain_namespace(
...
def test_video_to_dict(video, video_data):
    ...
    for key, value in video_dict.items():
        if key in list_type_keys:
            assert isinstance(value, list), "{} should be a list, but it's {}".format(key, value)
