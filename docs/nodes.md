...
Each node has the following attributes:
  ...
  - __grade_levels__ (list[str]): The educational level(s) the content is appropriate for (this field is referred to as `level` in the Kolibri Studio UI). Expects a list of constants from `le_utils.constants.labels.levels`. Example: `from le_utils.constants.labels import levels; node.grade_levels = [levels.PROFESSIONAL]`. (optional)
  - __resource_types__ (list[str]): The type(s) of resource. Expects a list of constants from `le_utils.constants.labels.resource_type`. Example: `from le_utils.constants.labels import resource_type; node.resource_types = [resource_type.GAME]`. (optional)
  - __learning_activities__ (list[str]): The type(s) of learning activity the content represents. Expects a list of constants from `le_utils.constants.labels.learning_activities`. Example: `from le_utils.constants.labels import learning_activities; node.learning_activities = [learning_activities.READ]`. (optional)
  - __categories__ (list[str]): The subject category(ies) the content belongs to (this field is referred to as `category` in the Kolibri Studio UI). Expects a list of constants from `le_utils.constants.labels.subjects`. Example: `from le_utils.constants.labels import subjects; node.categories = [subjects.MATHEMATICS]`. (optional)
